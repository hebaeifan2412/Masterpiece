...
@section('content')
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="mb-0"><i class="fas fa-tasks me-3"></i>Assignments Managememt </h2>
                ...
                <tbody>
                    @forelse ($assignments as $index => $assignment)
                        <tr>
                            ...
                            <td>
                                <div class="mb-2"><strong>Open:</strong>
                                    {{ \Carbon\Carbon::parse($assignment->open_time)->format('Y-m-d H:i') }}</div>
                                <div><strong>Close:</strong>
                                    {{ \Carbon\Carbon::parse($assignment->close_time)->format('Y-m-d H:i') }}</div>
                            </td>
                            ...
                            <td class="d-flex gap-2  justify-content-center align-items-center ">
                                ...
                                <form id="delete-form-{{ $assignment->id }}"
                                    action="{{ route('teacher.assignments.destroy', $assignment->id) }}" method="POST"
                                ...
                                <div class="dropdown">
                                    <button class="btn btn-sm bb text-light fw-bold dropdown-toggle" type="button"
                                        ...
                                        @foreach ($assignment->classProfiles as $class)
                                            @php
                                                ...
                                                $submitted = $assignment->submissions
                                                    ->filter(function ($submission) use ($class) {
                                                        return $submission->student &&
                                                            $submission->student->class_id == $class->id;
                                                    })
                                                    ->count();
                                            @endphp
                                            <li>
                                                <a class="dropdown-item d-flex justify-content-between align-items-center"
                                                    ...
                                                    <span class="badge bg-primary ms-2">{{ $submitted }} /
                                                        {{ $totalStudents }}</span>
                                                </a>
                                            </li>
                                        @endforeach
